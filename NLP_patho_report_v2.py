...
part_indicator = ['right', 'left','transverse polyp', 'sigmoid polyp', 'cecal polyp'] #indicator setting to find body parts
Disease_indicator = open('/Users/kohgiheon/Desktop/NLP_of_Clinical_data/practice/sample_diagnosis.txt').read().lower() #indicator setting to find disease description
# Disease_dictionary = Disease_indicator.lower().split('\n')

ID_PATH = ['Test4.txt','Test6.txt','Test8.txt','Test10.txt','Test12.txt']
# 'De_ID_Path.txt','De_ID_Path_2.txt', 'De_ID_Path3.txt', 'De_ID_Path4.txt', 'DE_ID_PAth_new_1.txt', 'Test1.txt', 'Test2.txt', 'Test3.txt','Test5.txt','Test7.txt','Test9.txt','Test11.txt','Test13.txt','Test14.txt','Test15.txt',
...
for pwd in ID_PATH : # for loop for processing all test sample at one click
    ...
    for ind in alphabet :
        if str(ind) +'.' in Final_diagnosis :
            if ind != alphabet[-1] : #distinguish each index
                cut_by_index.append(makeSentence(Final_diagnosis[Final_diagnosis.index(str(ind) +'.') +1: Final_diagnosis.index(str(alphabet[alphabet.index(ind) +1]) +'.')]))
    ...
    for i in Text_desc :
        ...
        a = word_tokenize(i)
        for wd in a :
            if wd.lower() in Disease_indicator :
                s.append(wd)
    ...
    for i in coredesc :
        w = word_tokenize(i)
        for ww in w :
            if ww in string.punctuation or ww.lower() in ['with', 'of', 'before', 'about', 'for', 'at', 'in' ] :
                w.pop(w.index(ww))
        if len(w) > 0 :
    ...
    print('Done')
    # print(len(body), len(proc), len(coredesc), len(grade))
    # print(grade)
