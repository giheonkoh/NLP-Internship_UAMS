...
ID_PATH = ['De_ID_Path.txt','De_ID_Path_2.txt', 'De_ID_Path3.txt', 'De_ID_Path4.txt', 'DE_ID_PAth_new_1.txt','Test1.txt', 'Test2.txt', 'Test3.txt','Test4.txt','Test5.txt','Test6.txt','Test7.txt','Test8.txt','Test9.txt', 'Test10.txt','Test11.txt','Test12.txt','Test13.txt','Test14.txt','Test15.txt']
...
for pwd in ID_PATH : # for loop for processing all test sample at one click
    ...
    DI = word_tokenize(Disease_indicator)
    for w in DI :
        if w in list(stop_words) or w in list(string.punctuation) :
            DI.pop(DI.index(w))
    ...
    for i in Text_desc :
        ...
        for w in word :
            if w not in list(string.punctuation) and w not in stop_words and w not in DI:
                DI.append(w)
    ...
    pd.DataFrame(DI).to_csv('/Users/kohgiheon/Desktop/NLP_of_Clinical_data/practice/sample_diagnosis.txt', index=False, header=False)
    print('Done')
