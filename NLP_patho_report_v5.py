...
ID_PATH = [ 'DE_ID_PAth_new_1.txt','De_ID_Path.txt' ,'De_ID_Path_2.txt', 'De_ID_Path3.txt', 'De_ID_Path4.txt','Test1.txt', 'Test2.txt', 'Test3.txt','Test4.txt','Test5.txt','Test6.txt','Test7.txt','Test8.txt','Test9.txt', 'Test10.txt','Test11.txt','Test12.txt','Test13.txt','Test14.txt','Test15.txt']
...
for pwd in ID_PATH : # for loop for processing all test sample at one click
    ...
    for sent in sentences : #process by sentence
        ...
        for catch in CD_unit: #Extract caridinal quantity information with its unit and description
            ...
            elif catch in symptom_indicator or UMLS.split('\n') :
    ...
    ### Description
    coredesc = []
    for sample in Text_desc :
        words = word_tokenize(sample.strip().lower())
        # words = words[words.index('-'):]

        for wd in words :
            if wd in stop_words or wd in list(string.punctuation) + ['-'] :
                words.pop(words.index(wd))
        ...
        for i in range(len(good)) :
            for k in range(6) :
                if any(makeSentence(good[i: i+k]).strip() in  s for s in UMLS.split('\n')) :
                    if makeSentence(good[i:i+k]) != '' :
                        w = makeSentence(good[i:i+k])
            lt.append(w.strip())
        lt = Find_unique(lt)

        minus = []
        for i in Find_unique(lt) :
            for ii in Find_unique(lt) :
                if i != ii :
                    if ii in i :
                        minus.append(ii)

        for i in Find_unique(minus) :
            lt.remove(i)
        ...
        res = []
        for wd in lt :
            r = list(filter(lambda x: wd in x, UMLS.split('\n')))
            res.append(r)

        errorate = []
        for i in range(len(lt)) :
            for k in res[i] :
                denominator = len(k.split())
                match = 0
                if len(word_tokenize(lt[i])) >=2 :
                    for ct in range(denominator) :
                        for c in range(len(word_tokenize(lt[i]))) :
                            if word_tokenize(lt[i])[c] == k.split()[ct] :
                                match +=1
                else :
                    for ct in range(denominator) :
                        if word_tokenize(lt[i]) == k.split()[ct] :
                            match +=1

                errorate.append([lt[i],k,match/denominator])

                # print(k.split()[ct])
                # print(word_tokenize(lt[i])[ct])
        ...
        if d == '' :
            if '.' not in sample.split() :
                ...
                o = word_tokenize(sample)
                d = makeSentence(o[:o.index('.')])

        coredesc.append(d.strip())
    ###
    print(coredesc)
...
print(database)
database.to_excel (r'/Users/kohgiheon/Desktop/result.xlsx', index = None, header=True)
